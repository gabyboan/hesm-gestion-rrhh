-- Add RPCs used by legajo-digital to manage the initial balance for the
-- francos hour bank.
--
-- The initial balance is stored as a regular francos_movimientos row with a
-- reserved motivo so existing saldo calculations continue to include it.

create unique index if not exists francos_movimientos_saldo_inicial_unico
on public.francos_movimientos (dni, carrera_id)
where anulado = false
...
$function$;
revoke execute on function public.rpc_francos_saldo_inicial_actual(bigint, bigint)
from public, anon;
revoke execute on function public.rpc_francos_saldo_inicial(bigint, bigint, date, integer, text)
from public, anon;
revoke execute on function public.rpc_francos_saldo_inicial_eliminar(bigint, bigint)
from public, anon;
grant execute on function public.rpc_francos_saldo_inicial_actual(bigint, bigint)
to authenticated;
grant execute on function public.rpc_francos_saldo_inicial(bigint, bigint, date, integer, text)
to authenticated;
grant execute on function public.rpc_francos_saldo_inicial_eliminar(bigint, bigint)
to authenticated;
